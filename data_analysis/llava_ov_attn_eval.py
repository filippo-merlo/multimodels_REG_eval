...
def aggregate_llm_attention(attn):
    ...
    for layer in attn:
        ...
        del layer_attns, attns_per_head, layer, vec
...
def aggregate_vit_attention_subtract_avg(attn, attn_avg, select_layer=-2):
    ...
    # Select the attention map for the specified layer
    selected_layer = attn[select_layer]
    layer_attns = selected_layer.squeeze(0)  # Remove batch dimension
    attns_per_head = layer_attns.mean(dim=0)  # Average over heads
    
    # Normalize the attention map
    vec_normalized = attns_per_head / attns_per_head.sum(-1, keepdim=True)
    
    # Subtract average attention and apply ReLU to keep non-negative values
    result = torch.relu(vec_normalized - attn_avg)

    # Move result to CPU to free GPU memory
    result = result.cpu()

    # Explicitly clean up memory
    del selected_layer, layer_attns, attns_per_head, vec_normalized
